...
class TicTacToe:
    ...
    def check_win(self):
        for i in range(3):
            if (
                self.buttons[i][0]['text'] == self.buttons[i][1]['text'] == self.buttons[i][2]['text'] == self.current_player
        ...
        if (
            self.buttons[0][0]['text'] == self.buttons[1][1]['text'] == self.buttons[2][2]['text'] == self.current_player
    ...
    def place_symbol(self, row, column):
        ...
        if clicked_button['text'] == "" and not self.win:
            ...
            self.check_win()
            self.switch_player()
            if not self.win and self.current_player == PLAYER_O:
                self.ai_turn()

    def ai_turn(self):


        # Choisissez une case au hasard pour l'IA
        empty_buttons = [(row, column) for column in range(3) for row in range(3) if self.buttons[column][row]['text'] == ""]
        if empty_buttons:
            row, column = random.choice(empty_buttons)
            self.buttons[column][row].config(text=PLAYER_O)
            self.check_win()
            self.switch_player()
